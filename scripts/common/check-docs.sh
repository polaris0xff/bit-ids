...
command -v git >/dev/null 2>&1 || {
  printf 'check-docs: git not found\n' >&2
  exit 2
}
git rev-parse --show-toplevel >/dev/null 2>&1 || {
  printf 'check-docs: not a git repository\n' >&2
  exit 2
}
command -v awk >/dev/null 2>&1 || {
  printf 'check-docs: awk not found\n' >&2
  exit 2
}
SELF=check-docs
...
[ -z "$FILES" ] && {
  printf 'check-docs: no markdown files in scope\n' >&2
  exit 2
}

TMP="${TMPDIR:-/tmp}/.checkdocs.$$"
mkdir -p "$TMP" || {
  printf 'check-docs: cannot write to %s\n' "$TMP" >&2
  exit 2
}
