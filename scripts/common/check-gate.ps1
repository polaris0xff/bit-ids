...
# ⛔ REPORTED AS SKIPS, NEVER OMITTED, for the same reason. check-store plants a
# symbolic link and a named pipe in a disposable tree, and neither is available
# to an unprivileged Windows session; check-corpus shares its harness. ⚠ The
# rules they prove are not Linux-only and the Rust suite exercises every one of
# them on both lanes; what this lane does not do is plant them against a real
# filesystem. CI-03 owns the Windows runner.
