...
command -v curl >/dev/null 2>&1 || {
  printf 'mine-repo: curl not found\n' >&2
  exit 2
}
command -v git >/dev/null 2>&1 || {
  printf 'mine-repo: git not found\n' >&2
  exit 2
}
...
mkdir -p "$DEST/api" || {
  printf 'mine-repo: cannot write to %s\n' "$DEST" >&2
  exit 2
}
...
if [ "$ROUTE" = "proxy" ]; then
  ...
  if [ "$_c" = "200" ]; then
    CONTROL_OK="reachable ($CONTROL answered 200)"
  else CONTROL_OK="⛔ UNREACHABLE ($CONTROL answered $_c). A 404 below means nothing."; fi
else
  if gh api "repos/$CONTROL" >/dev/null 2>&1; then
    CONTROL_OK="reachable ($CONTROL answered)"
