...
command -v git >/dev/null 2>&1 || {
  printf 'check-markers: git not found\n' >&2
  exit 2
}
command -v awk >/dev/null 2>&1 || {
  printf 'check-markers: awk not found\n' >&2
  exit 2
}
git rev-parse --show-toplevel >/dev/null 2>&1 || {
  printf 'check-markers: not a git repository\n' >&2
  exit 2
}
...
cd "$REPO_ROOT" || {
  printf 'check-markers: cannot enter %s\n' "$REPO_ROOT" >&2
  exit 2
}
...
mkdir -p "$TMP" || {
  printf 'check-markers: cannot write to %s\n' "$TMP" >&2
  exit 2
}
