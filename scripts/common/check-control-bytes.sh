...
command -v git >/dev/null 2>&1 || {
  printf 'check-control-bytes: git not found\n' >&2
  exit 2
}
git rev-parse --show-toplevel >/dev/null 2>&1 || {
  printf 'check-control-bytes: not a git repository\n' >&2
  exit 2
}
command -v awk >/dev/null 2>&1 || {
  printf 'check-control-bytes: awk not found\n' >&2
  exit 2
}
REPO_ROOT=$(git rev-parse --show-toplevel)
cd "$REPO_ROOT" || {
  printf 'check-control-bytes: cannot enter %s\n' "$REPO_ROOT" >&2
  exit 2
}
