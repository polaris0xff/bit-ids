...
command -v gh >/dev/null 2>&1 || {
  printf 'check-remote-items: gh not found\n' >&2
  exit 2
}
command -v jq >/dev/null 2>&1 || {
  printf 'check-remote-items: jq not found\n' >&2
  exit 2
}
gh auth status >/dev/null 2>&1 || {
  printf 'check-remote-items: gh is not authenticated\n' >&2
  exit 2
}
...
mkdir -p "$TMP" || {
  printf 'check-remote-items: cannot write to %s\n' "$TMP" >&2
  exit 2
}
