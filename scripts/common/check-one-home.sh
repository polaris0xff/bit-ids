...
command -v git >/dev/null 2>&1 || {
  printf 'check-one-home: git not found\n' >&2
  exit 2
}
command -v awk >/dev/null 2>&1 || {
  printf 'check-one-home: awk not found\n' >&2
  exit 2
}
git rev-parse --show-toplevel >/dev/null 2>&1 || {
  printf 'check-one-home: not a git repository\n' >&2
  exit 2
}
REPO_ROOT=$(git rev-parse --show-toplevel)
cd "$REPO_ROOT" || {
  printf 'check-one-home: cannot enter %s\n' "$REPO_ROOT" >&2
  exit 2
}
...
mkdir -p "$TMP" || {
  printf 'check-one-home: cannot write to %s\n' "$TMP" >&2
  exit 2
}
